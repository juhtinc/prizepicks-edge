...
module.exports = async function handler(req, res) {
  ...
  const toCheck = scripts.filter(
    (s) => s.scheduledDate === checkDate && s.youtubeVideoId,
  );

  if (!toCheck.length) {
    return res
      .status(200)
      .json({ ok: true, message: "No videos to check for " + checkDate });
  }

  const videoIds = toCheck.map((s) => s.youtubeVideoId);
  let analytics;
  try {
    analytics = await getVideoAnalytics(
      videoIds,
      checkDate,
    ...
    return res
      .status(500)
      .json({ error: "Analytics API failed", detail: e.message });
  }
  ...
  for (const script of toCheck) {
    ...
    // Track performance by story type for future planning
    const perfKey = `lore:perf:${script.storyType}`;
    let typePerf = await kv.get(perfKey);
    typePerf = typePerf
      ? typeof typePerf === "string"
        ? JSON.parse(typePerf)
        : typePerf
      : { totalViews: 0, totalRetention: 0, count: 0, underperformers: 0 };
    typePerf.totalViews += views;
    typePerf.totalRetention += retention;
    typePerf.count++;
    if (isUnderperformer) typePerf.underperformers++;
    typePerf.avgViews = Math.round(typePerf.totalViews / typePerf.count);
    typePerf.avgRetention = (typePerf.totalRetention / typePerf.count).toFixed(
      1,
    );
    typePerf.failRate = (
      (typePerf.underperformers / typePerf.count) *
      100
    ).toFixed(0);
    await kv.set(perfKey, JSON.stringify(typePerf), 86400 * 180);

    if (isUnderperformer) {
      const reason = lowRetention ? "bad_hook" : "bad_title";

      const prompt = `This YouTube Short about ${script.playerName} (${script.storyType}) underperformed (${views} views, ${retention}% retention).

Original title: "${script.titleUsed || script.titleA}"
Original hook: "${script.hookLine}"
Story type: ${script.storyType}
Script word count: ${(script.script || "").split(/\\s+/).length}

Diagnose the SPECIFIC failure:
- If retention < 30%: Was the hook weak? Was the opening boring? Did it fail to create curiosity?
- If views < channel avg: Was the title not compelling? Was the thumbnail concept wrong? Was the topic niche?

Generate a completely different approach:
{"new_title":"...","new_hook":"...","diagnosis":"specific failure reason","change_rationale":"why this new approach will work","story_type_note":"should we make more or fewer of this story type?"}`;
  ...
  return res
    .status(200)
    .json({ ok: true, checkDate, channelAvgViews, results });
};
