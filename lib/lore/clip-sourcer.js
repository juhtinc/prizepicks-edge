...
const {
  calculateClipSlots,
  getStoryTemplate,
} = require("./lib/story-templates");
const { scrapePlayerClips, searchPlayerPhoto } = require("./lib/clip-scraper");
const {
  transformClipBatch,
  uploadTransformedClips,
} = require("./lib/clip-transformer");
const {
  generateStatOverlays,
  statOverlaysToCreatomate,
} = require("./lib/stat-overlays");
...
{"clips":[{"slot":1,"visual":"what viewer sees","search_query":"specific play search","clip_type":"gameplay|photo|graphic","matches_script":"quote from script"},...],"player_photo_search":"${script.playerName} ${script.playerSport} portrait"}`;
  ...
  if (useRealFootage) {
    ...
    const gameplaySlots = (clipPlan.clips || []).filter(
      (c) => c.clip_type === "gameplay",
    );
    ...
    for (const planned of clipPlan.clips || []) {
      ...
      if (planned.clip_type === "gameplay" && realClipIndex < uploaded.length) {
        ...
        // Track origin video + timestamp for the Tweak feature
        const scraped = scrapedClips[realClipIndex];
        if (scraped) {
          ytVideoId = scraped.videoId || null;
          ytStartTime = scraped.startTime ?? null;
        }
        realClipIndex++;
  ...
  try {
    ...
    if (!playerPhotoUrl) {
      ...
      playerPhotoUrl =
        (await searchPlayerPhoto(script.playerName, script.playerSport)) || "";
    }
  } catch (e) {
...
module.exports = async function handler(req, res) {
  ...
  if (batchId) {
    ...
    const results = await Promise.all(
      scripts.map(async (script) => {
        const { clipBriefs, statOverlays, playerPhotoSearch } =
          await sourceClipsForScript(script, rejectionPatterns);
        ...
        script.clipSourceStatus =
          script.clipsSourced > 0 ? "Auto-sourced" : "Manual";
  ...
  if (!rowId)
    return res.status(400).json({ error: "rowId or batchId required" });
