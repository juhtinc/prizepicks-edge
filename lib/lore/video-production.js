...
// Words that get GOLD emphasis (stats, achievements, positive)
const GOLD_RE = /^(\d[\d,.]*|greatest|only|first|last|record|goat|legend|mvp|finals|championship|hall of fame|blocks?|points?|rebounds?|assists?|steals?|all-star|rookie|season|history|career|average)$/i;
// Words that get RED emphasis (dramatic, negative, shocking)
const RED_RE = /^(never|nobody|impossible|insane|worst|banned|fired|traded|died|injured|failed|robbed|forgotten|zero|none|nothing)$/i;
// Combined for backward compatibility
const EMPHASIS_RE = /^(\d[\d,.]*|never|nobody|impossible|insane|greatest|worst|only|first|last|record|goat|legend|banned|fired|traded|mvp|finals|championship|hall of fame|blocks?|points?|rebounds?|assists?|steals?|all-star|rookie|season|history|career|average|died|injured|failed|robbed|forgotten|zero|none|nothing)$/i;
...
function buildCaptionGroupsFromTimestamps(wordTimestamps) {
  ...
  function flushGroup() {
    ...
    const hasGold = words.some(w => GOLD_RE.test(w.word.replace(/[.,!?;:'"()-]/g, "")));
    const hasRed = words.some(w => RED_RE.test(w.word.replace(/[.,!?;:'"()-]/g, "")));
    groups.push({
      ...
      emphasisColor: hasRed ? "red" : hasGold ? "gold" : "white",
    });
    currentGroup = [];
  }
...
module.exports = async function handler(req, res) {
  ...
  // Always preserve the LAST sentence (closing question / CTA)
  const voWords = voiceoverText.split(/\s+/).filter(Boolean);
  if (voWords.length > 135) {
    const sentences = voiceoverText.split(/(?<=[.!?])\s+/);
    const lastSentence = sentences[sentences.length - 1] || "";
    const lastWords = lastSentence.split(/\s+/).filter(Boolean).length;
    const bodyTarget = 130 - lastWords; // reserve room for CTA

    let trimmed = "";
    let count = 0;
    for (let i = 0; i < sentences.length - 1; i++) {
      const sWords = sentences[i].split(/\s+/).filter(Boolean).length;
      if (count + sWords > bodyTarget) break;
      trimmed += (trimmed ? " " : "") + sentences[i];
      count += sWords;
    }
    // Append the closing CTA sentence
    trimmed += (trimmed ? " " : "") + lastSentence;
    count += lastWords;
    voiceoverText = trimmed;
    log.steps.script_trim = `${voWords.length} → ${count} words (CTA preserved)`;
  }
  ...
  let videoUrl = null;
  try {
    // Fetch player photo if not already set
    if (!script.playerPhotoUrl) {
      try {
        const { getPlayerImage } = require("./lib/player-images");
        const img = await getPlayerImage(script.playerName, script.playerSport);
        if (img && img.url) {
          script.playerPhotoUrl = img.url;
          log.steps.playerPhoto = "fetched";
        }
      } catch (e) {
        log.steps.playerPhoto = "failed: " + e.message;
      }
    }
    ...
    const source = await composeVideo({
      ...
      sfxPlacements: sfxPlacements.length > 0 ? sfxPlacements : null,
      targetDuration,
    });
