 ...
 * Groups 4-5 words, breaks at sentence boundaries, marks emphasis words.
 * Returns groups with per-word emphasis info for rich text rendering.
 */
function buildCaptionGroupsFromTimestamps(wordTimestamps) {
  ...
  for (let i = 0; i < wordTimestamps.length; i++) {
    ...
    const groupFull = currentGroup.length >= 5;
    const isLast = i === wordTimestamps.length - 1;
    // Also break at 4 words if next word starts a new clause (comma, dash)
    const nextIsClause = !isLast && /^[—\-]/.test(wordTimestamps[i + 1]?.word || "");
    const goodBreak = currentGroup.length >= 4 && (atSentenceEnd || nextIsClause);

    if (groupFull || goodBreak || (atSentenceEnd && currentGroup.length >= 2) || isLast) {
      // Don't leave a single word orphan at the end of a sentence
      if (isLast && currentGroup.length === 1 && groups.length > 0) {
        // Append to previous group instead
        const prev = groups[groups.length - 1];
        prev.text += " " + currentGroup[0].word;
        prev.duration = Math.round((currentGroup[0].end - prev.start + 0.15) * 100) / 100;
        prev.words.push({
          word: currentGroup[0].word,
          isEmphasis: EMPHASIS_RE.test(currentGroup[0].word.replace(/[.,!?;:'"()-]/g, "")),
        });
        currentGroup = [];
        continue;
      }
      ...
      groups.push({
        ...
        duration: Math.round((end - start + 0.2) * 100) / 100,
        words,
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!musicUrl || musicUrl.includes("drive.google.com/sports-lore")) {
      try {
        const { getMoodForStory } = require("./lib/music");
        const mood = getMoodForStory(script.storyType, script.script);
        const { selectFromLibrary } = require("./lib/music");
        const selected = selectFromLibrary(mood);
        musicUrl = selected.trackUrl;
        log.steps.music = `auto-selected: ${mood.mood}`;
      } catch (e) {
