...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Let the composer build segments from the script text + story template
    // (it splits sentences across segments proportionally)
    const source = await composeVideo({
      script,
      voiceoverUrl: voUrl,
      musicTrackUrl: musicUrl,
      segments: null,  // null = composer builds segments with text from script
    });
