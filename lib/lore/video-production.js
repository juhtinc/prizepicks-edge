...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Calculate target duration from voiceover word count (~165 WPM)
    const wordCount = voiceoverText.split(/\s+/).filter(Boolean).length;
    const estimatedVoiceDuration = Math.ceil((wordCount / 165) * 60);
    const targetDuration = Math.max(estimatedVoiceDuration, 55);
