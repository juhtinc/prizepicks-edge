...
module.exports = async function handler(req, res) {
  ...
  // Duration calculated here, passed to captions and composer
  const wordCount = voiceoverText.split(/\s+/).filter(Boolean).length;
  const estimatedVoiceDuration = Math.ceil((wordCount / 145) * 60);
  const targetDuration = Math.min(Math.max(estimatedVoiceDuration, 55), 59);

  let captionData = [];
  try {
    const captions = generateCaptions(voiceoverText, targetDuration, 145);
    captionData = captionsToCreatomate(captions);
    log.steps.captions = `${captionData.length} caption groups generated, ${targetDuration}s target`;
  } catch (e) {
