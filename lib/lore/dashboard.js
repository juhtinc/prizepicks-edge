...
async function logCost(service, amount, description) {
  try {
    ...
    costs = costs ? (typeof costs === 'string' ? JSON.parse(costs) : costs) : [];
    costs.push({ service, amount, description, date: new Date().toISOString() });
...
module.exports = async function handler(req, res) {
  ...
  if (action === "cost-data") {
    ...
    const costs = data ? (typeof data === 'string' ? JSON.parse(data) : data) : [];
  ...
  if (action === "log-cost" && req.method === "POST") {
    ...
    if (!service || amount == null) return res.status(400).json({ error: "service and amount required" });

    const kv = require("../../api/_kv");
    let costs = await kv.get("lore:costs:log");
    costs = costs ? (typeof costs === 'string' ? JSON.parse(costs) : costs) : [];
    costs.push({ service, amount: parseFloat(amount), description, date: new Date().toISOString() });
  ...
  if (action === "vps-health") {
    ...
    try {
      ...
      const resp = await fetch(`${clipApiUrl}/health`, { signal: controller.signal });
  ...
  if (action === "generate-thumbnails" && req.method === "POST") {
    ...
    const titles = script.allTitles || [script.titleA, script.titleB].filter(Boolean);
    ...
    return res.status(200).json({ thumbnails, playerPhoto, playerName: script.playerName });
  }
  ...
  if (action === "get-comments") {
    ...
    const script = await getScript(rowId);
    if (!script || !script.youtubeVideoId) return res.status(200).json({ comments: [] });
    ...
    try {
      ...
      if (!access_token) return res.status(200).json({ comments: [], error: "No access token" });
      ...
      const resp = await fetch(`https://www.googleapis.com/youtube/v3/commentThreads?${params}`, {
  ...
  if (action === "music-presets") {
    ...
    return res.status(200).json({ presets: presets ? (typeof presets === 'string' ? JSON.parse(presets) : presets) : [] });
  }
  ...
  if (action === "save-music-preset" && req.method === "POST") {
    ...
    presets = presets ? (typeof presets === 'string' ? JSON.parse(presets) : presets) : [];
    const existing = presets.findIndex(p => p.name === name);
    if (existing >= 0) {
      presets[existing] = { name, url, r2Url: r2Url || presets[existing].r2Url, savedAt: new Date().toISOString() };
    } else {
  ...
  if (action === "cache-music" && req.method === "POST") {
    ...
    try {
      ...
      if (!resp.ok) return res.status(500).json({ error: `Source returned ${resp.status}` });
  ...
  if (action === "edit-script" && req.method === "POST") {
    ...
    if (!rowId || !field || value == null) return res.status(400).json({ error: "rowId, field, value required" });

    const allowed = ["hookLine", "script", "commentBait", "titleA", "titleB", "scheduledDate", "scheduledPostTime", "musicTrack", "musicMood"];
    ...
    script[field] = value;
    await saveScript(rowId, script);
    return res.status(200).json({ ok: true, field, wordCount: (script.script || "").split(/\s+/).filter(Boolean).length });
  }
  ...
  if (action === "regen-script" && req.method === "POST") {
    ...
    try {
      ...
      const result = await askClaudeJSON(`You are rewriting a YouTube Shorts script about ${script.playerName} (${script.playerSport}, ${script.storyType}).
...
Rewrite the script incorporating the user's feedback. Keep it STRICTLY under 125 words (the voiceover must fit in 60 seconds — ElevenLabs speaks at ~2 words per second with natural pauses). Keep the same structure (hook + narrative + kicker with closing question). Count your words carefully.

Return JSON:
{"hookLine":"...","script":"...","titles":["title1","title2","title3","title4","title5"],"commentBait":"..."}`, { maxTokens: 1500 });
  ...
  if (action === "generate-titles" && req.method === "POST") {
    ...
    try {
      const { askClaudeJSON } = require("./lib/claude");
      const result = await askClaudeJSON(`Generate 5 YouTube Shorts title options for this sports story.
...
Return JSON: {"titles":["title1","title2","title3","title4","title5"]}`);
      ...
      if (titles.length < 2) return res.status(500).json({ error: "Too few titles" });
  ...
  if (action === "upload-photo" && req.method === "POST") {
    ...
    if (!rowId || !imageBase64) return res.status(400).json({ error: "rowId and imageBase64 required" });
  ...
  if (action === "youtube-search") {
    ...
    try {
      ...
      if (!useKey) {
        ...
        if (!accessToken) return res.status(500).json({ error: "Failed to get YouTube access token" });
      }
      ...
      const resp = await fetch(`https://www.googleapis.com/youtube/v3/search?${params}`, { headers });
      const data = await resp.json();
  ...
  if (action === "generate-voiceover" && req.method === "POST") {
    ...
    try {
      ...
      if (result.audioDuration && result.audioDuration > TARGET_MAX) {
        ...
        await logCost("ElevenLabs", 0.30, `Voiceover retry ${clampedSpeed.toFixed(2)}x: ${script.playerName}`);
      }
      ...
      script.voiceoverSpeed = result.audioDuration > TARGET_MAX ? null : (result.speed || 1.0);
      await saveScript(rowId, script);
      await logCost("ElevenLabs", 0.30, `Voiceover: ${script.playerName} (${result.audioDuration?.toFixed(1)}s)`);
