...
module.exports = async function handler(req, res) {
  ...
  if (action === "render-vps" && req.method === "POST") {
    ...
    try {
      const resp = await fetch(`${clipApiUrl}/render-video`, {
        method: "POST",
      ...
      if (!resp.ok)
        throw new Error(`VPS: ${(await resp.text()).slice(0, 200)}`);
      const result = await resp.json();

      if (result.ok && result.url) {
        script.renderUrl = result.url;
        await saveScript(rowId, script);
        return res
          .status(200)
          .json({ ok: true, url: result.url, size: result.size });
      } else {
        return res.status(500).json({ error: result.error || "render failed" });
      }
    } catch (e) {
      return res.status(500).json({ error: e.message });
    }
