 ...
 * Dashboard API for the ColdVault admin interface.
...
module.exports = async function handler(req, res) {
  ...
  if (action === "player-photo") {
    ...
    const wikiHeaders = {
      "User-Agent": "ColdVaultDashboard/1.0 (contact@coldvault.com)",
    };
    const wikiNames = [
      name,
      `${name} (basketball)`,
  ...
  // VPS render progress status
  if (action === "vps-render-status") {
    const clipApiUrl = process.env.CLIP_API_URL;
    if (!clipApiUrl) return res.status(200).json({ rendering: false });
    try {
      const resp = await fetch(`${clipApiUrl}/render-status`, {
        signal: AbortSignal.timeout(5000),
      });
      if (resp.ok) return res.status(200).json(await resp.json());
      return res.status(200).json({ rendering: false });
    } catch {
      return res.status(200).json({ rendering: false });
    }
  }
  ...
  // Render callback — VPS calls this when render is complete
  if (action === "render-callback" && req.method === "POST") {
    const { rowId, url, size, secret } = req.body || {};
    if (secret !== process.env.CRON_SECRET)
      return res.status(401).json({ error: "unauthorized" });
    if (!rowId || !url)
      return res.status(400).json({ error: "rowId and url required" });

    const { saveScript } = require("./lib/kv-lore");
    const script = await getScript(rowId);
    if (!script) return res.status(404).json({ error: "Script not found" });

    script.renderUrl = url;
    await saveScript(rowId, script);
    return res.status(200).json({ ok: true });
  }

  // Reconcile a render by probing R2 directly — self-heals if the VPS
  // callback failed to land (network blip, stale code, etc.).
  if (action === "reconcile-render" && req.method === "POST") {
    const { rowId } = req.body || {};
    if (!rowId) return res.status(400).json({ error: "rowId required" });
    ...
    if (script.renderUrl) {
      return res.status(200).json({ ok: true, url: script.renderUrl });
    }

    const R2_BASE = "https://pub-86aa1c96eda04a8099526017d95dbb8f.r2.dev";
    const candidateUrl = `${R2_BASE}/${rowId}/rendered.mp4`;
    try {
      ...
      if (headResp.ok) {
        const url = `${candidateUrl}?t=${Date.now()}`;
        script.renderUrl = url;
        await saveScript(rowId, script);
      ...
      return res
        .status(200)
        .json({ ok: false, reason: "not-ready", status: headResp.status });
    } catch (e) {
      return res.status(200).json({ ok: false, reason: e.message });
    }
  }

  // Remove background from image via VPS rembg
  // Render video via VPS FFmpeg (free alternative to Creatomate)
  if (action === "render-vps" && req.method === "POST") {
    const { rowId } = req.body || {};
    if (!rowId) return res.status(400).json({ error: "rowId required" });
    ...
    // Build clip list with timing
    const briefs = script.clipBriefs || [];
    const displayOrder =
      script.clipDisplayOrder &&
      script.clipDisplayOrder.length === briefs.length
        ? script.clipDisplayOrder
        : briefs.map((_, i) => i + 1);
    const totalDuration = Math.min(script.voiceoverDuration || 58, 59);
    const clipDur = totalDuration / (displayOrder.length || 18);

    const clips = displayOrder.map((slot, i) => {
      const brief = briefs.find((c) => c.slot === slot);
      const R2_BASE = "https://pub-86aa1c96eda04a8099526017d95dbb8f.r2.dev";
      return {
        url: brief?.clipUrl || `${R2_BASE}/${rowId}/clip_${slot}.mp4`,
        start: i * clipDur,
        duration: clipDur,
      };
    });

    // Build captions from voiceover timestamps
    const captions = [];
    const wordTs = script.voiceoverTimestamps;
    if (wordTs && wordTs.length > 0) {
      const MAX_WORDS = 4;
      for (let i = 0; i < wordTs.length; i += MAX_WORDS) {
        const chunk = wordTs.slice(i, i + MAX_WORDS);
        const text = chunk.map((w) => w.word).join(" ");
        const start = chunk[0].start;
        const end = chunk[chunk.length - 1].end;
        const nextStart =
          i + MAX_WORDS < wordTs.length
            ? wordTs[i + MAX_WORDS].start
            : end + 0.5;
        captions.push({
          text,
          start,
    ...
    // Detect outro start (last sentence)
    let outroStart = null;
    const fullScript =
      (script.hookLine ? script.hookLine + ". " : "") + (script.script || "");
    const sentences = fullScript.split(/(?<=[.!?])\s+/).filter(Boolean);
    const lastSentence =
      sentences.length > 0 ? sentences[sentences.length - 1] : "";
    const searchWord = lastSentence
      .split(/\s+/)
      .find(
        (w) =>
          w.length > 3 &&
          !/^(the|and|was|that|this|with|from|have|been|were|they|their|than|what|when|will|would|could|should|into|just|also|more|some|only|over|then|them|these|those|which|about|after|before|does|did|has|had|its|his|her|our|your|who|how|all|but|not|for)$/i.test(
            w,
          ),
      );
    if (searchWord && wordTs) {
      const clean = searchWord.toLowerCase().replace(/[.,!?;:'"()\-—]/g, "");
      for (let i = wordTs.length - 1; i >= 0; i--) {
        if (
          wordTs[i].word.toLowerCase().replace(/[.,!?;:'"()\-—]/g, "") === clean
        ) {
          const candidateStart = wordTs[i].start;
          const outroDur = totalDuration - candidateStart;
          if (outroDur > 1.5 && outroDur < 12) outroStart = candidateStart;
          break;
        }
      }
    }

    // Send render request to VPS — await just long enough for the request to be received
    // VPS will call back to save the render URL when done
    const callbackUrl = `https://prizepicks-edge.vercel.app/api/lore?route=dashboard&action=render-callback`;
    try {
      const renderResp = await fetch(`${clipApiUrl}/render-video`, {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          musicUrl: script.musicTrack || null,
          captions,
        ...
        signal: AbortSignal.timeout(55000), // Vercel has 60s limit, abort at 55s
      });
      // If VPS responds within 55s (unlikely for full render, but possible for errors)
      const renderResult = await renderResp.json().catch(() => ({}));
      if (renderResult.ok && renderResult.url) {
        script.renderUrl = renderResult.url;
        await saveScript(rowId, script);
        return res.status(200).json({ ok: true, url: renderResult.url });
      }
      // VPS is still rendering — it will callback when done
      return res.status(200).json({
        ok: true,
        message: "Render started — check back in 2-5 minutes",
      });
    } catch (e) {
      // Timeout is expected — VPS is rendering in the background
  ...
  if (action === "remove-bg" && req.method === "POST") {
    const { imageBase64, imageUrl } = req.body || {};
    if (!imageBase64 && !imageUrl)
      return res
        .status(400)
        .json({ error: "imageBase64 or imageUrl required" });
    ...
    try {
      const resp = await fetch(`${clipApiUrl}/remove-bg`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          imageBase64,
      ...
      if (!resp.ok)
        throw new Error(`VPS: ${(await resp.text()).slice(0, 100)}`);
  ...
  if (action === "edit-script" && req.method === "POST") {
    ...
    const prevValue = script[field];
    script[field] = value;

    // Player swap invalidates clip approvals — every existing clip is about
    // the wrong person, so force a clean re-source on the next trigger.
    let clipsReset = 0;
    if (field === "playerName" && prevValue && prevValue !== value) {
      (script.clipBriefs || []).forEach((c) => {
        c.approved = false;
        c.rejected = false;
...
Return JSON: {"titles":["title1","title2","title3","title4","title5"]}`);
  ...
  // Swap two slots in display order (slots in KV stay fixed)
  if (action === "swap-clips" && req.method === "POST") {
    ...
    // Initialize display order if missing (default = slots in order)
    if (!script.clipDisplayOrder || script.clipDisplayOrder.length !== 18) {
      script.clipDisplayOrder = Array.from({ length: 18 }, (_, i) => i + 1);
    }

    const a = parseInt(slotA);
    const b = parseInt(slotB);
    const idxA = script.clipDisplayOrder.indexOf(a);
    const idxB = script.clipDisplayOrder.indexOf(b);
    if (idxA >= 0 && idxB >= 0) {
      script.clipDisplayOrder[idxA] = b;
      script.clipDisplayOrder[idxB] = a;
    }

    await saveScript(rowId, script);
    return res
      .status(200)
      .json({ ok: true, displayOrder: script.clipDisplayOrder });
  }

  // (legacy swap kept for reference, no longer used)
  if (action === "_legacy_swap_unused" && req.method === "POST") {
    const { rowId, slotA, slotB } = req.body || {};
    const { saveScript } = require("./lib/kv-lore");
    const script = await getScript(rowId);
    if (!script) return res.status(404).json({ error: "Script not found" });
  ...
  if (action === "clip-from-youtube" && req.method === "POST") {
    const { rowId, slot, videoId, startTime, duration, mirror } =
      req.body || {};
  ...
  if (action === "edit-clip-library" && req.method === "POST") {
    const { index, newName, newFolder, remove } = req.body || {};
    ...
    if (remove) {
      ...
      if (newName && library[index]) library[index].name = newName;
      if (newFolder !== undefined && library[index]) {
        library[index].folder = newFolder || "";
        // Also update playerName for backward compat
        if (newFolder) library[index].playerName = newFolder;
      }
    }
  ...
  // List unique folders in the clip library
  if (action === "list-folders") {
    ...
    const folders = {};
    library.forEach((c) => {
      const folder = c.folder || c.playerName || "Unsorted";
  ...
  // Create a new folder (just by adding a placeholder entry, removed when first real clip added)
  if (action === "create-folder" && req.method === "POST") {
    ...
    const exists = library.some((c) => (c.folder || c.playerName) === name);
    if (!exists) {
      library.push({
        name: "(empty folder)",
        playerName: name,
