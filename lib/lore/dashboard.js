...
Return JSON: {"titles":["title1","title2","title3","title4","title5"]}`);
  ...
  if (action === "generate-voiceover" && req.method === "POST") {
    ...
    try {
      ...
      // If too long, retry with speed-up to hit exactly 58s
      if (result.audioDuration && result.audioDuration > TARGET_MAX) {
        const speedUp = result.audioDuration / 58; // target 58s exactly
        const clampedSpeed = Math.min(Math.max(speedUp, 1.01), 1.25);
      ...
      // Final safety check — if still over 59s after retry, speed up one more time
      if (result.audioDuration && result.audioDuration > TARGET_MAX) {
        const finalSpeed = result.audioDuration / 57;
        result = await generateVoiceover(fullText, {
          speed: Math.min(finalSpeed, 1.3),
        });
        await logCost(
          "ElevenLabs",
          0.3,
          `Voiceover final retry: ${script.playerName}`,
        );
      }
