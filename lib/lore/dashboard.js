    ...
    try {
      const resp = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          model: "claude-sonnet-4-20250514",
          max_tokens: 500,
          messages: [{ role: "user", content: `Generate 5 YouTube Shorts title options for this sports story.
...
Return JSON only: {"titles":["title1","title2","title3","title4","title5"]}` }],
        }),
      });
      const data = await resp.json();
      const text = data.content?.[0]?.text || "";
      const jsonMatch = text.match(/\{[\s\S]*\}/);
      if (!jsonMatch) return res.status(500).json({ error: "No titles generated" });
  ...
  if (action === "youtube-search") {
    ...
    try {
      ...
      if (data.error) return res.status(500).json({ error: data.error.message || "YouTube API error" });
