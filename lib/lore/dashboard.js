...
Return JSON: {"titles":["title1","title2","title3","title4","title5"]}`);
  ...
  if (action === "upload-voiceover" && req.method === "POST") {
    ...
    if (!rowId || !audioBase64) return res.status(400).json({ error: "rowId and audioBase64 required" });
    ...
    try {
      ...
      if (script) {
        ...
        script.voiceoverTimestamps = null; // no word-level timestamps for uploaded audio
        script.voiceoverDuration = null; // unknown until played
        await saveScript(rowId, script);
      }
