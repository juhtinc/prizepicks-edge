...
Return JSON only:
{"hookLine":"...","script":"...","titleA":"...","titleB":"...","commentBait":"..."}` }],
        }),
      });
  ...
  // Update title A/B selection
  if (action === "update-title" && req.method === "POST") {
    const { rowId, titleUsed } = req.body || {};
    if (!rowId || !titleUsed) return res.status(400).json({ error: "rowId and titleUsed required" });

    const { saveScript } = require("./lib/kv-lore");
    const script = await getScript(rowId);
    if (!script) return res.status(404).json({ error: "Script not found" });

    script.titleUsed = titleUsed;
    await saveScript(rowId, script);
    return res.status(200).json({ ok: true, titleUsed });
  }
