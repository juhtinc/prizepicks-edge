...
async function renderVideo({ voiceoverUrl, musicTrackUrl, clipUrls, textOverlays }) {
  ...
  // Background footage — use first clip if available, otherwise set to empty string
  // to clear template placeholder variables like {{footage_url}}
  modifications["Background Footage"] = (clipUrls && clipUrls[0]) ? clipUrls[0] : "";

  // Text overlays — map to template element names (sanitize mojibake)
  if (textOverlays?.hook_text) modifications["Hook Text"] = sanitizeText(textOverlays.hook_text);
  if (textOverlays?.player_name) modifications["Player Name"] = sanitizeText(textOverlays.player_name);
