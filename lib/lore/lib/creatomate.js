...
/**
 * Render a video from a raw JSON source composition (no template_id).
 * Used by video-composer.js to send programmatically-built compositions.
 *
 * @param {Object} source - Full Creatomate source JSON (output_format, width, height, duration, elements)
 * @returns {{ renderId: string, url: string|null, status: string }}
 */
async function renderComposition(source) {
  const requestBody = { source };
  console.log("[creatomate] renderComposition request:", JSON.stringify(requestBody).slice(0, 500));

  const resp = await axios.post(`${BASE_URL}/renders`, requestBody, { headers: headers() }).catch(e => {
    const body = e.response?.data ? JSON.stringify(e.response.data) : "no body";
    throw new Error(`Creatomate ${e.response?.status}: ${body} | source keys: ${Object.keys(source).join(",")}`);
  });
  const render = resp.data?.[0];
...
module.exports = { renderThumbnail, renderVideo, renderComposition, getRenderStatus };
