...
async function renderVideo({ voiceoverUrl, musicTrackUrl, clipUrls, textOverlays }) {
  ...
  // Set all template elements — use empty string for missing values to clear
  // Liquid placeholders like {{footage_url}} that would otherwise cause errors
  modifications["Background Footage"] = (clipUrls && clipUrls[0]) || "";
  modifications["Hook Text"] = sanitizeText(textOverlays?.hook_text || "");
  modifications["Player Name"] = sanitizeText(textOverlays?.player_name || "");
  modifications["Player Photo"] = textOverlays?.player_photo || "";
  modifications["Story Caption"] = sanitizeText(textOverlays?.caption || "");
  modifications["Stat 1"] = "";
  modifications["Stat 2"] = "";
  modifications["Stat 3"] = "";
