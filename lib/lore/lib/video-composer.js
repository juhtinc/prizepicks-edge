...
async function composeVideo({ script, voiceoverUrl, musicTrackUrl, segments, captionGroups, targetDuration }) {
  ...
  if (hookStats.length > 0) {
    // Dark overlay behind hook stats for readability
    elements.push({
      type: "shape",
      track: hookStatTrack,
      time: 0,
    ...
    // Gold stat text — large, centered, with dark bg behind each
    for (let si = 0; si < Math.min(hookStats.length, 3); si++) {
      elements.push({
        ...
        font_family: FONT,
        font_weight: "900",
        font_size: hookStats.length === 1 ? 90 : 72,
        ...
        x_alignment: "50%",
        width: "85%",
        y: hookStats.length === 1 ? "42%" : (si === 0 ? "36%" : si === 1 ? "46%" : "56%"),
        shadow_color: "rgba(245,166,35,0.5)",
        shadow_blur: 40,
        animations: [
          { type: "scale", start_scale: "85%", duration: "30%", easing: "back-out" },
        ],
      });
    }
  }
