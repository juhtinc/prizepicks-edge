...
function isStatReveal(text) {
  ...
  if (
    /(\d[\d,.]+)\s*(points?|ppg|rpg|apg|bpg|spg|%|games?|seasons?|wins?|losses?)/i.test(
      t,
    )
  )
    return true;
 ...
 * @param {Array}  opts.sfxPlacements - SFX placement objects from generateSFXPlacements()
...
async function composeVideo({
  ...
  // Collect clip URLs from script, respecting display order if set
  const briefs = script.clipBriefs || [];
  const displayOrder =
    script.clipDisplayOrder && script.clipDisplayOrder.length === briefs.length
      ? script.clipDisplayOrder
      : briefs.map((_, i) => i + 1);
  const clipUrls = displayOrder
    .map((slot) => {
      const brief = briefs.find((c) => c.slot === slot);
      return brief?.clipUrl || brief?.pexelsUrl;
    })
    .filter(Boolean);
  ...
  // ── Track 1: Background footage — quick cuts (1.5-2.5s per clip) ──
  const bgTrack = trackCounter++;
  const CUT_DURATION = 3; // seconds per clip
  let clipIndex = 0;

  for (let i = 0; i < segs.length; i++) {
    const seg = segs[i];
    const segEnd =
      i === segs.length - 1 ? Math.max(seg.end, totalDuration) : seg.end;
    const segDuration = segEnd - seg.start;
    if (segDuration <= 0) continue;

    // Subdivide segment into multiple quick cuts
    const numCuts = Math.max(1, Math.ceil(segDuration / CUT_DURATION));
    const cutDur = segDuration / numCuts;

    for (let c = 0; c < numCuts; c++) {
      const cutStart = seg.start + c * cutDur;
      const videoEl = {
        type: "video",
        track: bgTrack,
        time: cutStart,
        duration: cutDur,
        source: pickFootageUrl(clipIndex, clipUrls, seg.clipCategory),
        loop: true,
        fit: "cover",
        width: WIDTH,
        height: HEIGHT,
  ...
  for (let i = 1; i < segs.length; i++) {
    elements.push({
      ...
      // Flash fades out
      animations: [{ type: "fade", fade_type: "out", duration: "100%" }],
    });
  }
  ...
  const hookStats =
    hookSources.match(
      /\d[\d,.]*\s*(?:points?|pts?|rebounds?|reb|assists?|ast|blocks?|blk|shots?|steals?|stl|ppg|rpg|apg|bpg|games?|seasons?|wins?|losses?|%)/gi,
    ) || [];
  // Deduplicate
  const uniqueHookStats = [
    ...new Set(hookStats.map((s) => s.trim().toUpperCase())),
  ];
  ...
  if (uniqueHookStats.length > 0) {
    ...
    const UNIT_ABBREV = {
      ...
      BLK: "BLK",
      SHOTS: "BLK",
      SHOT: "BLK", // "397 shots" in blocking context
      STEALS: "STL",
      STEAL: "STL",
      STL: "STL",
      GAMES: "GM",
  ...
  if (captionGroups && captionGroups.length > 0) {
    ...
    for (let gi = 0; gi < captionGroups.length; gi++) {
      ...
      const overlapsReveal = segs.some(
        (seg) =>
          isRevealSegment(seg) &&
          group.start < seg.end &&
          group.start + group.duration > seg.start,
      );
      ...
      elements.push({
        ...
        font_size: 48,
        fill_color:
          group.emphasisColor === "red"
            ? "#FF3333"
            : group.emphasisColor === "gold"
              ? BRAND_GOLD
              : "#ffffff",
        x_alignment: "50%",
        width: "70%",
    ...
    for (let i = 0; i < segs.length; i++) {
      ...
      elements.push({
        ...
        shadow_blur: 16,
        line_height: "135%",
        animations: [{ type: "fade", fade_type: "in", duration: "8%" }],
      });
    }
  }
  ...
  for (let i = 0; i < segs.length; i++) {
    ...
    elements.push({
      ...
      y: 100,
      fill_color: "rgba(245,166,35,0.5)",
      animations: [{ type: "fade", fade_type: "in", duration: "15%" }],
    });
  ...
  const STAT_RE =
    /(\d[\d,.]+)\s*(points?|pts?|rebounds?|reb|assists?|ast|blocks?|blk|shots?|steals?|stl|ppg|rpg|apg|bpg|%)/gi;
  ...
  if (captionGroups && captionGroups.length > 0) {
    for (const group of captionGroups) {
      ...
      // Desaturated dim overlay (B&W effect like Barkley reference video)
      elements.push({
        type: "shape",
      ...
      // Big stat text — RED for dramatic pop against desaturated footage
      elements.push({
        type: "text",
  ...
  if (playerPhotoUrl && playerName) {
    ...
    elements.push({
      ...
      border_radius: "5%",
      animations: [{ type: "fade", fade_type: "in", duration: "10%" }],
    });
    // Player photo
    elements.push({
      type: "image",
  ...
  // ── Track 9: Logo watermark "Cold Vault" top-left ──
  ...
  // ── (same Track 9): @ColdVaultYT watermark bottom-right ──
  elements.push({
    type: "text",
    track: uiTrack,
    time: 0,
  ...
  elements.push({
    ...
    y: HEIGHT - 3,
    fill_color: BRAND_GOLD,
    animations: [{ type: "wipe", wipe_direction: "right", duration: "100%" }],
  });
  ...
  // ── Track 13: Sound effects (whoosh, bass boom, crowd, dramatic hit) ──
  if (sfxPlacements && sfxPlacements.length > 0) {
    const sfxTrack = trackCounter++;
    for (const sfx of sfxPlacements) {
      if (!sfx.url || sfx.type === "music_drop") continue;
      elements.push({
        type: "audio",
        track: sfxTrack,
        time: Math.max(0, sfx.time),
        duration: sfx.duration || 1,
        source: sfx.url,
        volume: `${sfx.volume || 30}%`,
      });
    }
  }

  // ── Outro card — replaces last clip(s) with branded end card ──
  // Detect the last sentence start time from caption groups
  if (captionGroups && captionGroups.length > 0) {
    // Find the start of the last sentence by looking for the last group
    // that starts a new sentence (after a period, or the final few groups)
    const lastGroup = captionGroups[captionGroups.length - 1];
    // Walk backwards to find where the kicker sentence begins
    let kickerStart = lastGroup.start;
    for (let i = captionGroups.length - 1; i >= 0; i--) {
      const g = captionGroups[i];
      const prevG = captionGroups[i - 1];
      // A new sentence starts after a period or after a gap > 0.5s
      if (!prevG || (prevG.text && /[.!?]$/.test(prevG.text.trim()))) {
        kickerStart = g.start;
        break;
      }
      // Also break if there's a long pause (new sentence boundary)
      if (prevG && g.start - (prevG.start + prevG.duration) > 0.4) {
        kickerStart = g.start;
        break;
      }
      kickerStart = g.start;
    ...
    // Only add outro if the kicker is within the last ~8 seconds
    const outroDuration = totalDuration - kickerStart;
    if (outroDuration > 1.5 && outroDuration < 10) {
      const outroTrack = trackCounter++;

      // Black background — covers all footage underneath
      elements.push({
        type: "shape",
        track: outroTrack,
        time: kickerStart,
      ...
      // Player name in gold
      const playerName = (script.playerName || "").toUpperCase();
      if (playerName) {
        elements.push({
          type: "text",
          track: outroTrack,
          ...
          y_alignment: "50%",
          z_index: 92,
          shadow_color: `rgba(245,166,35,0.15)`,
          shadow_blur: 40,
          animations: [
            {
              type: "slide",
            ...
            { type: "fade", fade_type: "in", duration: "20%" },
          ],
        });
      }
      ...
      elements.push({
        type: "shape",
        track: outroTrack,
        time: kickerStart + outroDuration * 0.4,
        duration: outroDuration * 0.6,
        width: 200,
        height: 2,
        x: WIDTH / 2,
        y: HEIGHT * 0.58,
        fill_color: `rgba(245,166,35,0.4)`,
        z_index: 92,
        animations: [
          { type: "wipe", wipe_direction: "right", duration: "40%" },
        ],
      });
      ...
      elements.push({
        type: "text",
        track: outroTrack,
        time: kickerStart + outroDuration * 0.5,
        ...
        letter_spacing: "15%",
        z_index: 92,
        animations: [{ type: "fade", fade_type: "in", duration: "40%" }],
      });
    }
  }
...
function buildSegmentsFromScript(script, templateSegments) {
  ...
  const totalDuration = templateSegments.reduce(
  ...
  for (const seg of templateSegments) {
    ...
    const segSentences = sentences.slice(
      sentenceIdx,
    ...
    results.push({
      ...
      type: isRevealSegment({ name: seg.name, clipCategory: seg.clipCategory })
        ? "reveal"
        : "footage",
    });
  }
