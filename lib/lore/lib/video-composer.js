...
const DEFAULT_STOCK_CLIPS = [
  ...
  "https://videos.pexels.com/video-files/4753987/4753987-hd_1080_1920_25fps.mp4",
...
function isRevealSegment(segment) {
  const revealNames = ["reveal", "kicker", "final", "verdict", "question"];
...
/**
 * Extract a displayable stat number from segment text for reveal overlays.
 * Looks for patterns like "100 points", "28 for 32", big numbers, etc.
 */
function extractStatFromText(text) {
  if (!text) return null;
  // Match numbers with optional units
  const patterns = [
    /(\d[\d,.]+)\s*(points?|pts?|games?|seasons?|rebounds?|assists?|blocks?|steals?|%)/i,
    /(\d[\d,.]+)\s*(?:for|\/)\s*(\d[\d,.]+)/i,  // "28 for 32"
    /(\d[\d,.]+)/,  // fallback: first number
  ];
...
async function composeVideo({ script, voiceoverUrl, musicTrackUrl, segments }) {
  ...
  for (let i = 0; i < segs.length; i++) {
    ...
    if (isRevealSegment(seg)) {
      // Reveal segments: solid dark background with gradient
      elements.push({
        type: "shape",
        track: bgTrack,
        time: seg.start,
      ...
      // Normal segments: stock video footage, looped
      elements.push({
        type: "video",
        track: bgTrack,
        time: seg.start,
  ...
  // ── Track 2: Gradient overlay (bottom fade for text readability) ──
  ...
  elements.push({
    ...
    gradient: [
      ...
      { offset: "50%", color: "rgba(0,0,0,0.3)" },
  ...
  for (let i = 0; i < segs.length; i++) {
    ...
    // Skip caption for reveal segments (they get the big stat overlay instead)
    if (isRevealSegment(seg) && extractStatFromText(seg.text)) continue;
    ...
    elements.push({
      ...
      x: WIDTH / 2,
      y: HEIGHT * 0.55,
      text_shadow: "0 2px 8px rgba(0,0,0,1)",
      line_height: "140%",
      animations: [
  ...
  for (let i = 0; i < segs.length; i++) {
    ...
    elements.push({
      ...
      x: WIDTH / 2,
      y: HEIGHT * 0.45,
      text_shadow: "0 0 50px rgba(245,166,35,0.4)",
      animations: [
  ...
  elements.push({
    ...
    y: 45,
    letter_spacing: "3px",
    border_color: "rgba(245,166,35,0.2)",
    border_width: 1,
    border_radius: 6,
    padding: "4px 10px",
  });
