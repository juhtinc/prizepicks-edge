...
async function composeVideo({ script, voiceoverUrl, musicTrackUrl, segments, captionGroups, statOverlays, targetDuration }) {
  ...
  if (captionGroups && captionGroups.length > 0) {
    // Use word-by-word caption groups from the captions module
    for (const group of captionGroups) {
      ...
      elements.push({
        ...
        font_family: FONT,
        font_weight: "800",
        font_size: group.hasHeroStat ? 72 : 48,
  ...
  if (statOverlays && statOverlays.length > 0) {
    ...
    for (const overlay of statOverlays) {
      ...
      if (overlay.type === "hero") {
        ...
        const num = parts ? parts[1] : overlay.content;
        ...
        // popup or other — small stat card
        elements.push({
          type: "text",
          track: statTrack,
          time: overlay.time,
