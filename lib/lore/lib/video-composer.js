...
async function composeVideo({ script, voiceoverUrl, musicTrackUrl, segments, captionGroups, targetDuration }) {
  ...
  for (let i = 0; i < segs.length; i++) {
    ...
    // Dim hook footage when stats will be overlaid
    if (i === 0) {
      videoEl.color_overlay = "rgba(0,0,0,0.6)";
    }
  ...
  const hookStats = hookSources.match(/\d[\d,.]*\s*(?:points?|pts?|rebounds?|reb|assists?|ast|blocks?|blk|steals?|stl|ppg|rpg|apg|games?|seasons?|%)/gi) || [];
  // Deduplicate
  ...
  if (uniqueHookStats.length > 0) {
    ...
    for (let si = 0; si < Math.min(uniqueHookStats.length, 2); si++) {
      ...
      const baseY = uniqueHookStats.length === 1 ? 42 : (si === 0 ? 34 : 50);
      ...
      elements.push({
        ...
        font_family: FONT,
        font_weight: "900",
        font_size: uniqueHookStats.length === 1 ? 160 : 120,
