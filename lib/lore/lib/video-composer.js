 ...
 */
async function composeVideo({ script, voiceoverUrl, musicTrackUrl, segments, captionGroups, statOverlays, targetDuration }) {
  ...
  const captionStatTrack = trackCounter++; // gold stat line below white text
  if (captionGroups && captionGroups.length > 0) {
    ...
    for (let gi = 0; gi < captionGroups.length; gi++) {
      ...
      // ── HOOK SECTION: show stat cards instead of caption text ──
      if (group.start < hookEndTime) {
        // Extract stat patterns from hook text (e.g. "26 points", "16 rebounds", "397 blocks")
        const statMatches = group.text.match(/\d[\d,.]*\s*(?:points?|pts?|rebounds?|reb|assists?|ast|blocks?|blk|steals?|stl|games?|seasons?|%)/gi);
        if (statMatches && statMatches.length > 0) {
          // Show each stat as a quick card
          const cardDuration = Math.min(hookEndTime - group.start, 2.5);
          for (let si = 0; si < statMatches.length; si++) {
            const stat = statMatches[si].trim();
            const parts = stat.match(/^([\d,.]+)\s*(.*)/);
            const num = parts ? parts[1] : stat;
            const unit = parts ? parts[2].toUpperCase() : "";
            elements.push({
              type: "text",
              track: captionStatTrack,
              time: group.start + (si * 0.3),
        ...
        continue; // skip regular caption for hook
      }
      ...
      if (group.hasEmphasis && group.words) {
        // Split into white narration text + gold stat text on separate lines
        const whiteWords = [];
        const goldWords = [];
        for (const w of group.words) {
          if (w.isEmphasis) {
            goldWords.push(w.word);
          } else {
            whiteWords.push(w.word);
          }
        }
        const whiteText = whiteWords.join(" ");
        const goldText = goldWords.join(" ");

        // White narration line
        if (whiteText.trim()) {
          elements.push({
            type: "text",
            track: captionTrack,
        ...
        if (goldText.trim()) {
          elements.push({
            ...
            width: "75%",
            y: "45%",
            shadow_color: "rgba(245,166,35,0.3)",
            shadow_blur: 20,
            line_height: "135%",
          });
        }
      } else {
        // No emphasis — single white line
        elements.push({
          type: "text",
          track: captionTrack,
          time: group.start,
  ...
  // ── Track 6b: Stat overlays — styled like reveal overlays from the preview ──
  // Each stat gets: dimming overlay + decorative gold lines + big stat + context
  if (statOverlays && statOverlays.length > 0) {
    const statDimTrack = trackCounter++;
    const statContentTrack = trackCounter++;
    for (const overlay of statOverlays) {
      if (!overlay.time || !overlay.content) continue;
      const dur = overlay.duration || 2;

      // Dark overlay to dim footage behind the stat
      elements.push({
        type: "shape",
        track: statDimTrack,
        time: overlay.time,
        ...
        animations: [
          { type: "fade", fade_type: "in", duration: "10%" },
          { type: "fade", fade_type: "out", duration: "10%", start_time: "85%" },
        ],
      });
      ...
      // Parse stat: try to split "397 BLK" into number + unit
      const parts = overlay.content.match(/^([\d,.]+)\s*(.*)/);
      const statNum = parts ? parts[1] : overlay.content;
      const statUnit = parts ? parts[2] : "";

      // Big gold stat number
      elements.push({
        type: "text",
        track: statContentTrack,
        time: overlay.time,
        ...
        font_family: FONT,
        font_weight: "900",
        font_size: statNum.length > 4 ? 100 : 160,
        fill_color: BRAND_GOLD,
        x_alignment: "50%",
        width: "85%",
        y: "42%",
