 ...
 */
async function composeVideo({ script, voiceoverUrl, musicTrackUrl, segments, captionGroups, sfxPlacements, targetDuration }) {
  ...
  const CUT_DURATION = 2; // seconds per clip for quick-cut pacing
  let clipIndex = 0;
  ...
  for (let i = 0; i < segs.length; i++) {
    ...
    for (let c = 0; c < numCuts; c++) {
      ...
      const videoEl = {
        ...
        height: HEIGHT,
        animations: [
          { type: "scale", start_scale: "100%", end_scale: "112%", duration: "100%" },
        ],
      };
  ...
  if (captionGroups && captionGroups.length > 0) {
    ...
    for (let gi = 0; gi < captionGroups.length; gi++) {
      ...
      elements.push({
        ...
        font_weight: "800",
        font_size: 48,
        fill_color: group.emphasisColor === "red" ? "#FF3333" : group.emphasisColor === "gold" ? BRAND_GOLD : "#ffffff",
        x_alignment: "50%",
        width: "70%",
        x: "43%",
        y: "42%",
