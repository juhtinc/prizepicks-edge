...
async function composeVideo({ script, voiceoverUrl, musicTrackUrl, segments, captionGroups, targetDuration }) {
  ...
  // Build hook text from caption groups that fall in the hook window
  const hookText = (captionGroups || [])
    .filter(g => g.start < hookEndTime)
    .map(g => g.text)
    .join(" ");

  // Extract stat patterns (e.g. "26 points", "16 rebounds", "397 blocks")
  const hookStats = hookText.match(/\d[\d,.]*\s*(?:points?|pts?|rebounds?|reb|assists?|ast|blocks?|blk|steals?|stl|ppg|rpg|apg|games?|seasons?|%)/gi) || [];

  if (hookStats.length > 0) {
    // Show stats stacked vertically in the center with blurred footage behind
    for (let si = 0; si < Math.min(hookStats.length, 3); si++) {
      elements.push({
        type: "text",
        track: hookStatTrack,
        time: 0.2 + (si * 0.15),
        duration: hookEndTime - 0.2,
        text: hookStats[si].trim().toUpperCase(),
        font_family: FONT,
        font_weight: "900",
        font_size: hookStats.length === 1 ? 80 : 60,
        fill_color: BRAND_GOLD,
        x_alignment: "50%",
        width: "80%",
        y: hookStats.length === 1 ? "42%" : (si === 0 ? "36%" : si === 1 ? "44%" : "52%"),
        shadow_color: "rgba(245,166,35,0.4)",
        shadow_blur: 30,
        animations: [
          { type: "scale", start_scale: "90%", duration: "25%", easing: "back-out" },
        ],
      });
    }
  }
  ...
  // ── Track 12: Background music (single continuous element) ──
  ...
  if (musicTrackUrl) {
    elements.push({
      ...
      volume: "20%",
      animations: [
        // Silent during hook, fade in after
        { type: "fade", fade_type: "in", duration: `${((hookEnd + 2) / totalDuration * 100).toFixed(1)}%` },
      ],
    });
  }
