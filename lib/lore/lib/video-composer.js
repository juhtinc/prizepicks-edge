...
async function composeVideo({ script, voiceoverUrl, musicTrackUrl, segments, captionGroups, statOverlays, targetDuration }) {
  ...
  const captionEmphasisTrack = trackCounter++; // overlay track for gold emphasis words
  if (captionGroups && captionGroups.length > 0) {
    for (const group of captionGroups) {
      ...
      // Base text: all words in white with dark background
      elements.push({
        type: "text",
        track: captionTrack,
        time: group.start,
      ...
      // Gold overlay: emphasis words only (non-emphasis replaced with spaces)
      if (group.hasEmphasis && group.words) {
        const emphasisText = group.words
          .map(w => w.isEmphasis ? w.word : " ".repeat(w.word.length))
          .join(" ");

        elements.push({
          type: "text",
          track: captionEmphasisTrack,
          time: group.start,
          duration: group.duration,
          text: emphasisText,
          font_family: FONT,
  ...
  // ── Track 6b: Stat overlays (hero stats, tickers, popups from clip-sourcer) ──
  if (statOverlays && statOverlays.length > 0) {
    const statTrack = trackCounter++;
    for (const overlay of statOverlays) {
      if (!overlay.time || !overlay.content) continue;
      const dur = overlay.duration || 2;

      if (overlay.type === "hero") {
        // Giant centered stat — show full content (number + unit together)
        // Position in upper third to avoid caption overlap
        elements.push({
          type: "text",
          track: statTrack,
          time: overlay.time,
          duration: dur,
          text: overlay.content.toUpperCase(),
          font_family: FONT,
          font_weight: "900",
          font_size: 100,
          fill_color: BRAND_GOLD,
      ...
      } else if (overlay.type === "ticker") {
        // Stat line — positioned above captions
        elements.push({
          type: "text",
          track: statTrack,
          time: overlay.time,
          ...
          y: "35%",
          animations: [
            { type: "slide", direction: "up", distance: "2%", duration: "15%" },
            { type: "fade", fade_type: "in", duration: "10%" },
          ],
        });
      } else {
        // popup — small stat card in upper area
        elements.push({
          type: "text",
          track: statTrack,
          time: overlay.time,
          ...
          y: "30%",
          animations: [
            { type: "scale", start_scale: "90%", duration: "15%", easing: "back-out" },
            { type: "fade", fade_type: "in", duration: "10%" },
          ],
        });
      }
