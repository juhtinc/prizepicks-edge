...
async function composeVideo({ script, voiceoverUrl, musicTrackUrl, segments, captionGroups, targetDuration }) {
  ...
  if (hookStats.length > 0) {
    // Dark dimmed background for reveal card look
    elements.push({
      type: "shape",
      track: hookStatTrack,
      time: 0,
      duration: hookEndTime,
      width: WIDTH,
      height: HEIGHT,
      x: WIDTH / 2,
      y: HEIGHT / 2,
    ...
    // Parse each stat into number + unit for card layout
    for (let si = 0; si < Math.min(hookStats.length, 2); si++) {
      const stat = hookStats[si].trim();
      const parts = stat.match(/^([\d,.]+)\s*(.*)/);
      const num = parts ? parts[1] : stat;
      const unit = parts ? parts[2].toUpperCase() : "";

      // Vertical positioning: single stat centered, two stats stacked
      const baseY = hookStats.length === 1 ? 42 : (si === 0 ? 34 : 50);
      ...
      elements.push({
        ...
        track: hookStatTrack,
        time: 0.1,
        duration: hookEndTime - 0.1,
        text: num,
        font_family: FONT,
        font_weight: "900",
        font_size: hookStats.length === 1 ? 160 : 120,
