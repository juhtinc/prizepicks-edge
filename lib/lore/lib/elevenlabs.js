...
function addSSMLPacing(text, options = {}) {
  ...
  const dramaticOpeners =
    /(?<=[.!?]\s)(But |And then |No one |Nobody |Never |However |That's when |Imagine |Unfortunately )/g;
  ...
  for (const word of revealWords) {
    ...
    ssml = ssml.replace(
      regex,
      '<break time="300ms"/><prosody rate="90%">$1</prosody>',
    );
  }
  ...
  if (sentences.length > 1) {
    sentences[sentences.length - 1] =
      `<prosody rate="85%">${sentences[sentences.length - 1]}</prosody>`;
    ssml = sentences.join(" ");
  }
...
async function generateVoiceover(text, options = {}) {
  ...
  // Upload audio to R2 for permanent storage
  let audioUrl;
  try {
    ...
    const key = `voiceovers/${audioId}.mp3`;
    ...
    audioUrl = `https://pub-86aa1c96eda04a8099526017d95dbb8f.r2.dev/${key}`;
  } catch (e) {
    // Fallback to KV if R2 fails
    const kv = require("../../../api/_kv");
    const audioId = `vo-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    await kv.set(`lore:audio:${audioId}`, audioBase64, 86400);
    audioUrl = `https://prizepicks-edge.vercel.app/api/lore?route=audio&id=${audioId}`;
  }
...
/**
 * Generate voiceover at a specific speed using the regular TTS endpoint.
 * Used when the with-timestamps endpoint doesn't support the speed parameter.
 * Returns just the audio URL (no timestamps).
 */
async function generateVoiceoverFast(text, speed = 1.0) {
  const apiKey = process.env.ELEVENLABS_API_KEY;
  if (!apiKey) return null;
  ...
  await kv.set(`lore:audio:${audioId}`, base64, 86400);

  return `https://prizepicks-edge.vercel.app/api/lore?route=audio&id=${audioId}`;
}
...
module.exports = {
  generateVoiceover,
  generateVoiceoverFast,
  addSSMLPacing,
  getAudioPostProcessingFilter,
  deriveWordTimestamps,
};
