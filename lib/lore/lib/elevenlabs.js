...
async function generateVoiceover(text, options = {}) {
  ...
  await kv.set(`lore:audio:${audioId}`, base64, 86400); // 24h TTL

  // Return a URL that Creatomate can fetch — our own endpoint serves the audio
  const host = process.env.VERCEL_URL || "prizepicks-edge.vercel.app";
  return `https://${host}/api/lore?route=audio&id=${audioId}`;
}
