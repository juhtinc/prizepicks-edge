...
async function generateVoiceover(text, options = {}) {
  ...
  // Add SSML pacing unless disabled
  const processedText = options.addPacing !== false ? addSSMLPacing(text, options) : text;
  ...
  const resp = await axios.post(
    `${BASE_URL}/text-to-speech/${voiceId}`,
    {
      ...
      model_id: modelId,
      voice_settings: {
        stability: 0.65,           // Higher = more consistent character (was 0.5)
        similarity_boost: 0.55,     // Lower = more expressive range (was 0.75)
        style: 0.3,                 // Storytelling energy (0 = neutral, 1 = max)
  ...
  try {
    ...
    console.error("[elevenlabs] Blob upload failed, falling back to base64:", e.message);
