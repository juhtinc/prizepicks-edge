...
async function generateVoiceover(text, options = {}) {
  ...
  // Upload to Vercel Blob (private store) and return a signed download URL
  const { put, getDownloadUrl } = require("@vercel/blob");
  ...
  // Generate a signed URL valid for 1 hour (enough for Creatomate to fetch)
