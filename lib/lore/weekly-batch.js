...
RULES:
- HARD LIMIT: 130 words MAXIMUM. Count carefully. Scripts over 130 words WILL be rejected. Aim for 120-130 words.
...
- THIS IS A HISTORICAL / EDUCATIONAL SPORTS CHANNEL. Focus on telling THIS player's story and achievements:
...
Return JSON: {"script":"...","word_count":<actual count>,"opinion_stance":"what stance you took and why","comment_bait":"the exact closing question"}`;

    const result = await askClaudeJSON(scriptPrompt, { maxTokens: 500 });

    // Enforce word count limit — trim last sentences if over 135 words
    let scriptText = result.script || "";
    const words = scriptText.split(/\s+/).filter(Boolean);
    if (words.length > 135) {
      // Trim to last complete sentence under 135 words
      const sentences = scriptText.split(/(?<=[.!?])\s+/);
      let trimmed = "";
      let count = 0;
      for (const sentence of sentences) {
        const sentenceWords = sentence.split(/\s+/).filter(Boolean).length;
        if (count + sentenceWords > 135) break;
        trimmed += (trimmed ? " " : "") + sentence;
        count += sentenceWords;
      }
      scriptText = trimmed || scriptText.split(/\s+/).slice(0, 130).join(" ") + ".";
    }
  ...
  if (phase === "clips") {
    ...
    if (process.env.GITHUB_PAT && rowIds.length > 0) {
      try {
        ...
        console.log(`[batch] GitHub Actions triggered for ${rowIds.length} scripts`);
      } catch (e) {
        ...
        } catch (e2) { console.error(`[batch] fallback clip sourcing failed:`, e2.message); }
      }
    } else {
