...
${await (async () => {
  const types = ['forgotten_legend','record_breaker','what_if','scandal','comeback','rivalry','draft_bust','underdog'];
  const lines = [];
  for (const t of types) {
    const d = await kv.get(`lore:perf:${t}`);
    if (d) {
      const p = typeof d === 'string' ? JSON.parse(d) : d;
      lines.push(`${t}: ${p.avgViews || 0} avg views, ${p.avgRetention || 0}% retention, ${p.failRate || 0}% fail rate (${p.count || 0} videos)`);
    }
  }
...
RULES:
- ABSOLUTE HARD LIMIT: 100 words MAXIMUM. This is critical — the voiceover MUST be under 59 seconds. At natural speaking pace with pauses, 100 words = ~55 seconds. Count EVERY word. Aim for 90-100 words.
...
Return JSON: {"script":"...","word_count":<actual count>,"opinion_stance":"what stance you took and why","comment_bait":"the exact closing question","loop_explanation":"how the closing question circles back to the opening","foreshadow_line":"the foreshadowing line and when it pays off"}`;
    ...
    if (words.length > 105) {
      ...
      scriptText = trimmed || scriptText.split(/\s+/).slice(0, 100).join(" ") + ".";
    }
