 ...
 *
 * Runs TWICE per week (Monday + Thursday) to produce 14 videos total.
 * Each batch generates 7 stories, scheduled as 2 per day:
 *   Batch A (Monday):    Mon AM, Mon PM, Tue AM, Tue PM, Wed AM, Wed PM, Thu AM
 *   Batch B (Thursday):  Thu PM, Fri AM, Fri PM, Sat AM, Sat PM, Sun AM, Sun PM
 *
...
Return JSON: {"script":"...","word_count":140,"opinion_stance":"what stance you took and why","comment_bait":"the exact closing question"}`;
    ...
    // Check if all scripts in the batch are done
    const batch = await getBatch(batchId);
    if (batch) {
      batch.scriptsCompleted = (batch.scriptsCompleted || 0) + 1;
      await saveBatch(batchId, batch);

      // If this is the last script, trigger the enhance phase
      if (batch.scriptsCompleted + 1 >= batch.videoCount) {
  ...
  // ── PHASE 3: Enhance — fan out metadata + hooks + music in parallel ──
  if (phase === "enhance") {
    const scripts = await getBatchScripts(batchId);
    if (!scripts.length) return res.status(404).json({ error: "No scripts in batch" });

    // Fire all enhance calls in parallel (fire-and-forget, each is its own function)
    const dispatched = [];
    for (const script of scripts) {
      const rid = script.rowId;
      dispatched.push(rid);

      // These run as separate serverless functions, so they don't count against our 60s
      axios.post(`${baseUrl}/api/lore?route=generate-metadata`, { rowId: rid }, { headers, timeout: 55000 })
        .catch(e => console.error(`[enhance] metadata failed ${rid}:`, e.message));
      axios.post(`${baseUrl}/api/lore?route=optimize-hook`, { rowId: rid }, { headers, timeout: 55000 })
        .catch(e => console.error(`[enhance] hook failed ${rid}:`, e.message));
      axios.post(`${baseUrl}/api/lore?route=select-music`, { rowId: rid }, { headers, timeout: 55000 })
        .catch(e => console.error(`[enhance] music failed ${rid}:`, e.message));
    }

    // Trigger clips phase after a delay to let enhance calls finish
    // (clips phase will run in ~55s, by which time enhance should be done)
    axios.post(
      `${baseUrl}/api/lore?route=weekly-batch&phase=clips&batch=${batchHalf}`,
      { batchId },
      { headers, timeout: 55000 }
    ).catch(e => console.error(`[enhance] clips dispatch failed:`, e.message));

    return res.status(200).json({
      ok: true, phase: "enhance", batchId,
      dispatched: dispatched.length,
      message: `Dispatched metadata + hook + music for ${dispatched.length} scripts in parallel`,
    });
  }
